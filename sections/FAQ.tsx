...
const FAQ = () => {
  ...
  return (
    <section id="faq" className="py-24 relative overflow-hidden bg-teal-900">
      ...
      {/* Background Layers - OTIMIZAÇÃO: transform-gpu para evitar repaint */}
          ...
          {faqs.map((item, index) => {
            ...
            return (
              <motion.div 
                ...
                viewport={{ once: true }}
                // OTIMIZAÇÃO: 'transform-gpu' isola o backdrop-blur para não travar a animação de altura
                className={`
                  border rounded-2xl overflow-hidden transition-all duration-300 backdrop-blur-md transform-gpu
                  ${isActive 
                    ? 'bg-white/15 border-teal-400/40 shadow-lg shadow-teal-900/20' 
              ...
              >
                ...
                >
                  <span className={`font-bold text-lg transition-colors ${isActive ? 'text-white' : 'text-teal-50'}`}>
                    {item.question}
                  </span>
                  <span className={`
                    p-2 rounded-full transition-colors shrink-0 ml-4
                    ${isActive ? 'bg-teal-400 text-teal-900' : 'bg-white/10 text-white'}
                  `}>
                ...
                <AnimatePresence initial={false}>
                  {isActive && (
                    <motion.div
                      ...
                      className="overflow-hidden" // Crucial para animação de altura funcionar bem
                    >
                      <div className="p-6 pt-0 text-teal-100 leading-relaxed border-t border-white/10 mt-2">
                        {item.answer}
                      </div>
                    </motion.div>
                  )}
